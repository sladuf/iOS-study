...
📌 vTable
- 클래스 마다 가지고 있는 메서드들의 포인터 값을 가지는 테이블입니다
...
4. 다운캐스팅을 하는 경우에는 실패 가능성이 있으므로 **as?** 또는 **as!** 를 사용합니다
5. as?는 옵셔널이 반환됩니다. as!는 옵셔널이 강제 추출되므로 다운캐스팅이 확실히 보장되는 경우에만 사용해야 합니다
```
🤔 캐스팅하면 타입이 바뀌지 않나요?
- 네, 예를들어 class Person {}, class Student: Person {}를 살펴보겠습니다
- let student = Student() as Persen을 하는 경우에는 업캐스팅되어 Student에서 새로 만든 프로퍼티와 메서드를 사용하는 것이 불가능합니다
- 하지만 type(of: student)는 여전히 Student입니다. 즉, type이 바뀌는 것이 아니라 범위를 전환하는 것입니다
```
